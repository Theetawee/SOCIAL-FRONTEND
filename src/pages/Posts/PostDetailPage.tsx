...
const PostDetailPage = () => {
    ...
    if (isPending) {
        ...
        if (isAxiosError(error) && error.response?.data.detail === "Not found.") {
            return (
                <Seo title={"Post not found!!  / Waanverse"} description={"Something went wrong."}>
                    <NotFound type="post"/>
                    </Seo>
            )
        } else {
                ...
                {post ? (
                    <>
                        <Seo
                            title={`${
                                post.account?.name
                            } on Waanverse: ${post.content.slice(0, 30)}...`}
                        ...
                        >
                            <section className="pt-4">
                                <article className="grid grid-cols-1 gap-y-4 rounded w-full">
                                    ...
                                    <hr className="border-t h-px border-gray-100 dark:border-gray-800" />
                                </article>
                            </section>
                            {/* <section className="">
